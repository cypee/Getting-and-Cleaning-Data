run_analysis<-function(){
    ...
    tidy_dataset<-rbind(test_subjects,train_subjects)
    ...
    for(i in 1:nrow(tidy_dataset)){
        tidy_dataset$Activity[i]<-activity_label[subject_activity[i,1],2]
    }
    ...
    tidy_dataset <-cbind(tidy_dataset[,1:2], tidy_dataset[, grepl("mean|std", colnames(tidy_dataset))])
    ...
    cat("\n### Please find the file: Tidy_Data.csv saved in your working directory ###")
